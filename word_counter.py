...
def merge(words, known_words):
    new_words = []
    for word in words:
        tag = nltk.pos_tag(word_tokenize(word.lower()))
        pos = get_wordnet_pos(tag[0][1])
        if pos:
            lemmatized_word = lmtzr.lemmatize(word.lower(), pos)
            ...
            if (lemmatized_word not in known_words) and (len(lemmatized_word) > 2):
